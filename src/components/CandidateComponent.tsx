...
const CandidateComponent = ({candidate}:any) => {
    ...
    const handleClickNextCandidate = async (buttonName: string) => {
        try {
            ...
            const addCandidateWithRelocation = {...newCandidate, relocation: "Unassigned", candidateAddAt: new Date().toISOString()};
            ...
            if (buttonName === 'btn-save') {
                dispatch(saveCandidates({...candidateData, relocation: "Unassigned", candidateAddAt: new Date().toISOString()}));
            }
        } catch (err) {
